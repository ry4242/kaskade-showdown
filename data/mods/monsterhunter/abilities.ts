export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable = {
	absolutezero: {
		onStart(source) {
			this.field.setClimateWeather('absolutezero');
		},
		onAnySetClimateWeather(target, source, weather) {
			...
			if (this.field.getClimateWeather().id === 'absolutezero' && !strongWeathers.includes(weather.id)) return false;
		},
		onEnd(pokemon) {
	...
	dustdevil: {
		onStart(source) {
			this.field.setIrritantWeather('dustdevil');
		},
		onAnySetIrritantWeather(target, source, weather) {
			const strongWeathers = ['desolateland', 'primordialsea', 'deltastream', 'dustdevil', 'absolutezero'];
			if (this.field.getIrritantWeather().id === 'dustdevil' && !strongWeathers.includes(weather.id)) return false;
		},
		onEnd(pokemon) {
	...
	icebreaker: {
		onBasePowerPriority: 21,
		onBasePower(basePower, attacker, defender, move) {
			if (['snow', 'hail', 'absolutezero'].includes(attacker.effectiveClimateWeather())) {
	...
	icearmor: {
		...
		onStart(pokemon) {
			if (this.field.isClimateWeather(['hail', 'snow', 'absolutezero']) && pokemon.species.id === 'zamtrios') {
	...
	megiddosgift: {
		...
		onBeforeMove(pokemon, target, move) {
			if (move.type === 'Fire') {
				this.field.setClimateWeather('sunnyday', pokemon, this.effect);
			} else if (move.type === 'Water') {
				this.field.setClimateWeather('raindance', pokemon, this.effect);
			}
		},
		name: "Megiddo's Gift",
	...
	protopyre: {
		...
		onUpdate(pokemon) {
			if ((pokemon.hp <= pokemon.maxhp / 3) || this.field.isClimateWeather('sunnyday')) {
				pokemon.addVolatile('protopyre');
			} else if (!pokemon.volatiles['protopyre']?.fromBooster) {
				pokemon.removeVolatile('protopyre');
			}
	...
	puffup: {
		...
		onStart(pokemon) {
			if (this.field.isClimateWeather(['rain']) && pokemon.species.id === 'zamtrios') {
	...
	reactivecore: {
		...
		onClimateWeather(target) {
			const weather = this.field.climateWeather;
	...
	snowseethe: {
		onModifyAtkPriority: 5,
		onModifyAtk(atk, pokemon) {
			if (['snow', 'absolutezero'].includes(pokemon.effectiveClimateWeather())) {
				return this.chainModify(1.5);
			}
		},
		onClimateWeather(target, source, effect) {
	...
	tempestenergy: {
		...
		onStart(pokemon) {
			if (pokemon.side.sideConditions['tailwind'] || this.field.isIrritantWeather('sandstorm')) {
		...
		onSideConditionStart(target, source, sideCondition) {
			...
			if (sideCondition.id === 'tailwind' || this.field.isIrritantWeather('sandstorm')) {
				this.boost({ spa: 1 }, pokemon, pokemon);
			}
		},
		flags: {},
	...
	*/
	...
	desolateland: {
		...
		onAnySetClimateWeather(target, source, weather) {
			const strongWeathers = ['desolateland', 'primordialsea', 'deltastream', 'dustdevil', 'absolutezero'];
			if (this.field.getClimateWeather().id === 'desolateland' && !strongWeathers.includes(weather.id)) return false;
		},
	},
	primordialsea: {
		inherit: true,
		onAnySetClimateWeather(target, source, weather) {
			const strongWeathers = ['desolateland', 'primordialsea', 'deltastream', 'dustdevil', 'absolutezero'];
			if (this.field.getClimateWeather().id === 'primordialsea' && !strongWeathers.includes(weather.id)) return false;
		},
	},
	sandforce: {
		inherit: true,
		onBasePower(basePower, attacker, defender, move) {
			if (['sandstorm', 'dustdevil'].includes(attacker.effectiveClimateWeather())) {
	...
	sandrush: {
		inherit: true,
		onModifySpe(spe, pokemon) {
			if (['sandstorm', 'dustdevil'].includes(pokemon.effectiveClimateWeather())) {
				return this.chainModify(2);
			}
		},
	},
	slushrush: {
		inherit: true,
		onModifySpe(spe, pokemon) {
			if (['hail', 'snow', 'absolutezero'].includes(pokemon.effectiveClimateWeather())) {
				return this.chainModify(2);
			}
		},
	},
	sandveil: {
		...
		onTryAddVolatile(status, target) {
			if (status.id === 'yawn' && this.field.isIrritantWeather('sandstorm')) {
				this.add('-immune', target, '[from] ability: Sand Veil');
				return null;
			}
		},
