export const Conditions: import('../../../sim/dex-conditions').ModdedConditionDataTable = {
	...
	dustdevil: {
		...
		onModifySpD(spd, pokemon) {
			if (pokemon.hasType('Rock') && this.field.isClimateWeather('dustdevil')) {
		...
		onClimateWeather(target) {
			if (this.field.climateWeatherState.source !== target) this.damage(target.baseMaxhp / 16);
		},
		onFieldEnd() {
	...
	absolutezero: {
		...
		onModifyDef(def, pokemon) {
			if (pokemon.hasType('Ice') && this.field.isClimateWeather('absolutezero')) {
				return this.modify(def, 1.5);
			}
		},
		onModifySpe(spe, pokemon) {
			if (this.field.climateWeatherState.source !== pokemon) return this.chainModify(0.75);
		},
		onFieldStart(field, source, effect) {
