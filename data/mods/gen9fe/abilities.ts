...
export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable = {
	...
	growthspurt: {
		...
		onResidual(pokemon) {
			if (pokemon.hp && !pokemon.item && this.dex.items.get(pokemon.lastItem).isBerry &&
				(this.field.isClimateWeather(['sunnyday', 'desolateland']) || this.randomChance(1, 2))) {
	...
	openingact: {
		...
		onPrepareHit(source, target, move) {
			const isItSunny = this.field.isClimateWeather('sunnyday');
			if (move.priority > 0) {
	...
	weightoflife: {
		...
		onUpdate(pokemon) {
			if (!(pokemon.volatiles['weightoflife']?.fromBooster ||
				pokemon.volatiles['weightoflife']?.fromWeightDiff) && !this.field.isClimateWeather('sunnyday')) {
				pokemon.removeVolatile('weightoflife');
			}
		},
		onAnyPrepareHit(source, target, move) {
	...
	ultraface: {
		...
		onClimateWeatherChange(pokemon, source, sourceEffect) {
			...
			if ((sourceEffect as Ability)?.suppressClimateWeather || !pokemon.hp) return;
			if (this.field.isClimateWeather(['hail', 'snowscape']) && pokemon.species.id === 'eisephalonnoface') {
	...
	sandsword: {
		...
		onBasePower(basePower, attacker, defender, move) {
			if (move.flags['slicing']) {
				...
				return this.chainModify(this.field.isIrritantWeather('sandstorm') ? 1.5 : 1.25);
			}
		},
		onImmunity(type, pokemon) {
			if (type === 'sandstorm') return false;
	...
	sandworm: {
		...
		onResidual(pokemon) {
			if (pokemon.hp && pokemon.status && (this.field.isIrritantWeather('sandstorm') || this.randomChance(33, 100))) {
