		...
		for (const pokemon of Object.keys(setsJSON)) {
			...
			for (const set of sets) {
				...
				for (let j = 0; j < rounds; j++) {
					...
					const moveSet = generator.randomMoveset(types, abilities, {}, species, true, false, movePool, role);
					...
					for (let i = 0; i < 8; i++) {
						...
						const movePool = set.movepool.map(m => dex.moves.get(m).id);
						const moveSet = generator.randomMoveset(types, abilities, teamDetails, species, false, false, movePool, role);
						for (const move of moveSet) moves.delete(move);
						if (!moves.size) break;
					}
					if (!moves.size) break;
		...
		for (const pokemon of Object.keys(setsJSON)) {
			...
			for (const set of sets) {
				...
				for (let j = 0; j < rounds; j++) {
					// Generate a moveset as the lead, teamDetails is always empty for this
					const movePool = set.movepool.map(m => dex.moves.get(m).id);
					const moveSet = generator.randomMoveset(types, abilities, {}, species, true, true, movePool, role);
					for (const move of moveSet) moves.delete(move);
					if (!moves.size) break;
					// Generate a moveset for each combination of relevant teamDetails
					for (let i = 0; i < 8; i++) {
						...
						// randomMoveset() deletes moves from the movepool, so recreate it every time
						const movePool = set.movepool.map(m => dex.moves.get(m).id);
						const moveSet = generator.randomMoveset(types, abilities, teamDetails, species, false, true, movePool, role);
						for (const move of moveSet) moves.delete(move);
						if (!moves.size) break;
					}
					if (!moves.size) break;
