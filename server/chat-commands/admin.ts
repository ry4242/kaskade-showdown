...
export const commands: Chat.ChatCommands = {
	...
	editbattle(target, room, user) {
		...
		case 'climate':
			...
			[value] = targets.map(toID);
			void battle.stream.write(`>eval battle.field.setClimateWeather('${value}', 'debug')`);
			break;
		case 'irritant':
			if (targets.length !== 1) {
				this.errorReply("Incorrect command use");
			...
			[value] = targets.map(toID);
			void battle.stream.write(`>eval battle.field.setIrritantWeather('${value}', 'debug')`);
			break;
		case 'energy':
			if (targets.length !== 1) {
				this.errorReply("Incorrect command use");
			...
			[value] = targets.map(toID);
			void battle.stream.write(`>eval battle.field.setEnergyWeather('${value}', 'debug')`);
			break;
		case 'strongwinds':
			if (targets.length !== 1) {
				this.errorReply("Incorrect command use");
			...
			[value] = targets.map(toID);
			void battle.stream.write(`>eval battle.field.setClearingWeather('${value}', 'debug')`);
			break;
		case 'terrain':
		case 't':
			if (targets.length !== 1) {
